...
import {ArrowLeft, ArrowRight, Calendar, Filter, Lock, Setting, User} from "@element-plus/icons-vue";
...
const role=ref()
onMounted(() => {
    const storeRole = sessionStorage.getItem('role')||localStorage.getItem('role');
    ...
    if (storeRole) {
      role.value=JSON.parse(storeRole);
    }else{
      ElMessage.warning('请先登录')
      router.push('/')
    }
  router.push(`/nurseIndex/hospitalizations`)
})
...
function goToMedicineManagePage(){
  router.push('/nurseIndex/medicine')
}
        ...
        <el-aside :width="isCollapsed ? '65px':'200px'" style="margin-right: 20px;background-color: #f3d19e" class="el-aside">
          ...
          <el-scrollbar>
            <el-menu :default-active="['1']" background-color="#f3d19e" >
              <!-- 医生信息管理菜单 -->
              <el-menu-item index="1" @click="goToHospitalizationsPage">
                <template #title>
