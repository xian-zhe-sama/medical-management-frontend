...
onMounted(()=>{
  ...
  const sessionDoctorId = JSON.parse(sessionStorage.getItem('doctorId'));
  doctorId.value=localDoctorId||sessionDoctorId
  form.value.doctorId=doctorId.value
...
const rule = ref({
  startDate: [
    {required: true, message: '选择开始时间',  trigger: ['blur', 'change']},
  ],
  endDate: [
...
<!--          <el-button type="primary"  @click="getScheduleByName" :icon="Search"></el-button>-->
    ...
    <el-dialog align-center v-model="dialogFormVisible" title="保存值班信息" width="500">
      <el-form :model="form" :rules="rule" ref="formRef">
        <el-form-item label="医生id" :label-width="formLabelWidth" prop="doctorId" >
          <el-input v-model="form.doctorId" type="text" disabled/>
        </el-form-item>
        <el-form-item label="开始时间" :label-width="formLabelWidth" prop="startTime">
          <el-date-picker v-model="form.startDate" type="datetime" placeholder="选择值班开始时间" />
        </el-form-item>
